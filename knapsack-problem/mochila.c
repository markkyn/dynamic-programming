...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < V_i_w.rows; i++)
    {
        // where W is the max capacity of the sack
        for (int w = 1; w < V_i_w.cols; w++)
        {
            if (w < items[i-1].weight)
            {
                int x = get_value(&V_i_w, i - 1, w);
                set_value(&V_i_w, i, w, x);
            }
            else
            { // Fits
                int32_t wout_item = get_value(&V_i_w, i - 1, w - items[i-1].weight) + items[i-1].value;
                int32_t with_item = get_value(&V_i_w, i - 1, w);

                int m = max(with_item, wout_item);
                set_value(&V_i_w, i, w, m);
            }
        }
    }
